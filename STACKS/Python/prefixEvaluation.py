...
def Solve(postfix):
    ...
    for x in postfix:
        if x.isdigit() :          # if x in digit
            ...
            Stack.append( str(Opr[x](int(B), int(A))) )         # evaluate the 2 values poped from stack & push result to stack
            print(x.rjust(8), ('push('+B+x+A+')').ljust(12), ','.join(Stack), sep = " | ")      # output in tabular format
